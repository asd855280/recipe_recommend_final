...
ImageComponent, BoxComponent, SpacerComponent, ButtonComponent, SeparatorComponent
)
...
def process_postback_event(event):
    ...
    # '''取出用戶Line的userID及username並存到資料庫'''
        ...
        try:
            ...
            line_bot_api.reply_message(
                event.reply_token,
                TextSendMessage(text="%s 您好!\n您可以點選'我的基本資料'來修改您的個人資料" % (user_profile.display_name)))
        ...
        try:
            ...
            def register1(userID):
                return render_template('register1.html', ID=userID)
        ...
        final_data = data0 + data1 + data2 + data3 + data4 + data5 + data6 + data7 + data8 + data9 + data10
        ...
        line_bot_api.reply_message(
            event.reply_token,
            [TextSendMessage(text="{}".format(final_data)), message])
...
#取出消息內User的資料
        ...
        try:
            ...
            def recipeCollect(userID):
                ...
                cur.close()
                conn.close()
                return render_template('recipe_love.html', recipe_favorite=u)


            @app.route('/delete/<insertID>', methods = ['GET'])
            def delete(insertID):
                conn = pymysql.connect(host=host, port = port, user= user, passwd=passwd, db=db)
                flash("成功刪除！")
                cur = conn.cursor()
                ...
                cur.close()
                conn.close()
                return redirect(url_for('recipeCollect'))
            ...
            def planCollect(userID):
                conn = pymysql.connect(host=host, port = port, user= user, passwd=passwd, db=db)
                cur = conn.cursor()
                sql = """SELECT * FROM plan_favorite WHERE userID='{}'""".format(userID)
                cur.execute(sql)
                u = cur.fetchall()
                cur.close()
                conn.close()
                return render_template('plan_love.html', plan_favorite=u)


            @app.route('/remove/<insertID>', methods = ['GET'])
            def remove(insertID):
                conn = pymysql.connect(host=host, port = port, user= user, passwd=passwd, db=db)
                flash("成功刪除！")
                cur = conn.cursor()
                ...
                cur.close()
                conn.close()
                return redirect(url_for('planCollect'))
        except:
            pass
        ...
        line_bot_api.reply_message(
                    ...
                    contents=BubbleContainer(
                        hero=ImageComponent(
                            url="https://img.ebcbuzz.com/images/1/4631/F_m.jpg",
                            size="full",
                            aspectRatio="1.51:1",
                            aspectMode="cover"
                        ),
...
# ============================ 飲食日記 ===================================================
        ...
        try:
            ...
            def record(userID):
                conn = pymysql.connect(host=host, port=port, user=user, passwd=passwd, db=db)
                cur = conn.cursor()
                sql = """SELECT * FROM diary WHERE userID='{}'""".format(userID)
                cur.execute(sql)
                u = cur.fetchall()
                cur.close()
                conn.close()
                return render_template('diary.html', diary=u)
            ...
            @app.route('/kill/<diary_id>', methods = ['GET'])
            def killdiary(diary_id):
                flash("日記刪除成功！")
                conn = pymysql.connect(host=host, port=port, user=user, passwd=passwd, db=db)
                cur = conn.cursor()
        ...
        contents=BubbleContainer(
            hero=ImageComponent(
                url="https://x8a7r9m5.stackpathcdn.com/wp-content/uploads/2017/05/food-symbolism-in-miyazaki-hayao-films-7.jpg",
                size="full",
                aspectRatio="1.51:1",
                aspectMode="cover"
            ),
